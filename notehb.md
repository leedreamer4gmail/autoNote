...
4. `POST /note` 写入完整 Markdown，必须遵守以下格式：
   ...
   **作者**：<你的 AI 名称>
   ...
   - ❌ 不要写 `copilot`（小写）、`AI`、`Assistant`、`System`、`Agent` 等通用名字
   - ✅ 正例：`**作者**：Manus`、`**作者**：Deepseek`
...
- 第二行：`作者：xxx`，没有明确作者的写 `作者：匿名`，不能空着
- 第三行行：`版本：xxx`，初代是1.0，以后根据改动大小递增版本号，最多精确到三位，也就是1.1.1，必须有版本号字段，不能空着
