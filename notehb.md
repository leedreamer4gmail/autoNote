...
| 层级 | 地址 | 说明 | 适用场景 |
|------|------|------|----------|
| **首选** | `https://leedreamer.cn/transnote/` | 走 Nginx 代理（HTTPS） | 日常使用，外网可直接访问 |
| **后备** | `http://8.219.6.216:8888` | 直连 transNote 端口 | 8888 端口在阿里云安全组放行后可用；Nginx 出问题时作为维修通道 |
| **本地** | `http://127.0.0.1:8888` | 服务器本机直连 | 仅在服务器内调试用（SSH 到服务器后） |

> ⚠️ **接入顺序：先试首选（Nginx），连不上再试直连 8888。**
>
> 8888 端口默认**未在阿里云安全组放行**，外网直连不通是正常的。如果 Nginx 代理也不通，可以手动去阿里云控制台放行 8888 端口作为维修通道。

### 鉴权

所有请求（`/health` 除外）需要 Header：
```
...
BASE = "https://leedreamer.cn/transnote"  # 首选 Nginx 代理（HTTPS），不通再试 http://8.219.6.216:8888
...
- 第二行：`作者：xxx`，**必须写实际创作这篇笔记的 AI 或人的名称**，填写规则：
  ...
  - 你是人类写的 → 写你的名字
  - ❌ 禁止写 `copilot`（全小写）、`AI`、`助手`、`Agent`、`System` 等通用词
...
| 20 | AI 在服务器上编辑文档时用 `sed` 做多行替换容易出错 | 文本含特殊字符、换行符、引号时，`sed` 替换命令难以正确转义，容易破坏文件 | **优先用 Python 脚本读写文件（`read()`→修改→`write()`），逻辑清晰且字符安全；脚本用完立即 `rm` 删除** |
| 20 | 外网直连 `http://8.219.6.216:8888` 超时不通 | 阿里云安全组未放行 8888 端口 | **首选走 Nginx 代理** `https://leedreamer.cn/transnote/`；8888 端口仅在安全组放行后用作维修通道 |
| 21 | AI 在服务器内部用外网 IP 调 transNote 反而超时/卡住 | 服务器内用外网 IP 访问自己需要安全组放行且经过公网 | **在服务器内调 transNote 用 `127.0.0.1:8888`**，不要用外网 IP |
...
1. AI 生成标准格式的 payload JSON 文件（`id` 可省略，前端自动分配）：
   ...
   ```
2. 用户把文件下载到本地，浏览器打开 `https://leedreamer.cn/transnote/`（首选，HTTPS）或 `http://8.219.6.216:8888/`（后备，需安全组放行）
...
| 项目 | 值 |
|------|-----|
| 首选地址 | `https://leedreamer.cn/transnote/`（Nginx 代理，HTTPS） |
...
BASE    = "https://leedreamer.cn/transnote"  # 首选 Nginx
HEADERS = {"X-Api-Key": "simpleNote888"}
...
```powershell
# 读接口：PowerShell OK
Invoke-RestMethod "https://leedreamer.cn/transnote/notes" -Headers @{"X-Api-Key"="simpleNote888"}
